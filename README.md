...
![クラス図の例](Class/_Template_Class.png)
...
| **コマンド** | `java -jar plantuml.jar -charset UTF-8 -tpng Class/_Template_Class.puml` → 同じフォルダに `.png` が出力（要 Java）<br>全図まとめて生成は `... -tpng "*/_Template_*.puml"` |
...
| ファイル | 図 | 主な記法 |
|----------|----|----------|
| [`_Template_Class.puml`](Class/_Template_Class.puml) | クラス図 | 可視性 `+ - # ~`、`abstract`/`interface`/`enum`、継承 `<\|--`／実装 `<\|..`／集約 `o--`／コンポジション `*--`／関連 `-->`／依存 `..>`、多重度 |
| [`_Template_Sequence.puml`](Sequence/_Template_Sequence.puml) | シーケンス図 | `->`同期/`-->`戻り/`->>`非同期、`activate`、`alt`/`opt`/`loop`、`note`、`==区切り==` |
| [`_Template_Activity.puml`](Activity/_Template_Activity.puml) | アクティビティ図 | `start`/`stop`、`:アクション;`、`if/else`、`repeat`、`fork`、スイムレーン |
| [`_Template_State.puml`](State/_Template_State.puml) | 状態遷移図 | `[*]`開始終了、`-->`遷移、複合状態、`entry/do/exit`、`<<choice>>` |
| [`_Template_UseCase.puml`](UseCase/_Template_UseCase.puml) | ユースケース図 | `actor`、`usecase`、`rectangle`境界、`<<include>>`/`<<extend>>` |
| [`_Template_Component.puml`](Component/_Template_Component.puml) | コンポーネント図 | `component [..]`、`interface`、提供/要求、`package` |
| [`_Template_Deployment.puml`](Deployment/_Template_Deployment.puml) | 配置図 | `node`/`database`/`cloud`、`artifact`、接続 |
| [`_Template_Object.puml`](Object/_Template_Object.puml) | オブジェクト図 | `object "名 : 型" { 属性=値 }`、`json` |
| [`_Template_Timing.puml`](Timing/_Template_Timing.puml) | タイミング図 | `robust`/`concise`/`binary`/`clock`、`@時刻` |
